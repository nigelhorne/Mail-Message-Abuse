...
sub make_raw_email {
	...
	my @rcvd = ref($h{received}) eq 'ARRAY'
		? @{ $h{received} }
		: ($h{received} // 'from ext.mail.example (ext.mail.example [198.51.100.42]) by mx.test (Postfix); Mon, 01 Jan 2024 00:00:00 +0000');
...
subtest 'Scenario 1: direct-to-MX spam — full pipeline' => sub {
	...
	install_stubs(
		...
		resolve  => { 'spamsite.example' => '198.51.100.99' },
		whois_ip => {
...
WHOIS
	...
	$a->parse_email(make_raw_email(
		received => 'from badactor (badactor [198.51.100.42]) by mx.test',
	...
	is $orig->{ip},		 '198.51.100.42',   'correct originating IP extracted';
	...
	is $urls[0]{ip},   '198.51.100.99',		'URL host resolved to IP';
	...
	# --- report() ---
	my $report = $a->report();
	like $report, qr/198\.51\.100\.42/,			'originating IP in report';
...
subtest 'Scenario 3: display-name spoofing and reply-to misdirection' => sub {
	...
	my $a = new_ok('Mail::Message::Abuse');
	$a->parse_email(make_raw_email(
		received  => 'from phishhost (phishhost [198.51.100.77]) by mx.test',
	...
	my $report = $a->report();
	like $report, qr/198\.51\.100\.77/,		   'phishing source IP in report';
...
subtest 'Scenario 5: URL shortener hides real destination' => sub {
	...
	my $a = new_ok('Mail::Message::Abuse');
	$a->parse_email(make_raw_email(
		received => 'from sender (sender [198.51.100.1]) by mx.test',
...
subtest 'Scenario 7: authentication failures — SPF, DKIM, DMARC all fail' => sub {
	...
	my $a = new_ok('Mail::Message::Abuse');
	$a->parse_email(make_raw_email(
		received => 'from forged (forged [198.51.100.5]) by mx.test',
...
subtest 'Scenario 8: trusted relay excluded; two external hops give high confidence' => sub {
	...
	install_stubs(
		rdns	=> sub {
			my (undef, $ip) = @_;
			return 'mail.attacker.example'	 if $ip eq '198.51.100.10';
			return 'relay.legitrelay.example'  if $ip eq '203.0.113.5';
			return undef;
		},
	...
	my $a = Mail::Message::Abuse->new(
		trusted_relays => ['203.0.113.0/24'],  # our own relay
	);
	$a->parse_email(make_raw_email(
		received => [
			# Top (most recent): our trusted relay accepted from external relay
			'from relay.legitrelay.example (relay.legitrelay.example [203.0.113.5]) by mx.test',
			# Middle: an external relay accepted from the attacker
			'from mail.attacker.example (mail.attacker.example [198.51.100.10]) by relay.legitrelay.example',
			# Bottom: the attacker sent directly from this IP
			'from attacker (attacker [198.51.100.10]) by mail.attacker.example',
		],
	...
	# The trusted relay (203.0.113.5) must be excluded
	ok $orig->{ip} ne '203.0.113.5', 'trusted relay IP excluded from origin';

	# Two non-trusted hops (198.51.100.10 appears twice) → high confidence
	is $orig->{confidence}, 'high', 'two external hops → high confidence';
	is $orig->{ip}, '198.51.100.10', 'attacker IP identified as origin';
	...
	install_stubs(
		...
		whois_ip => sub {
			...
			my %data = (
				...
				'198.51.100.1'=> { org => 'Sender ISP',	   abuse => 'abuse@sender.example' },
			);
			return $data{$ip} // {};
		},
		domain_whois => sub {
...
subtest 'Scenario 11: re-parsing replaces all state — no leakage between emails' => sub {
	...
	install_stubs(
		...
		resolve => { 'firstsite.example' => '198.51.100.10' },
	...
	$a->parse_email(make_raw_email(
		received => 'from first (first [198.51.100.10]) by mx.test',
	...
	is $orig1->{ip}, '198.51.100.10', 'first email: correct origin';
	...
	$a->parse_email(make_raw_email(
		received => 'from clean (clean [203.0.113.1]) by mx.test',
	...
	is $orig2->{ip}, '203.0.113.1',	're-parse: new origin IP';
...
subtest 'Scenario 12: clean legitimate email scores INFO — no false positives' => sub {
	...
	$a->parse_email(make_raw_email(
		received	 => 'from mail.verifiedcorp.example (mail.verifiedcorp.example [203.0.113.10]) by mx.test',
...
subtest 'Scenario 14: report() and abuse_report_text() consistent on same object' => sub {
	...
	install_stubs(
		...
		resolve  => { 'spammer.example' => '198.51.100.55' },
	...
	my $a = new_ok('Mail::Message::Abuse');
	$a->parse_email(make_raw_email(
		received => 'from spam (spam [198.51.100.55]) by mx.test',
...
subtest 'Scenario 16: webmail origin — X-Originating-IP fallback at low confidence' => sub {
	...
	my $orig = $a->originating_ip();
	ok defined $orig,					   'originating_ip returns a value';
	is $orig->{ip},		 '203.0.113.200','X-Originating-IP used as origin';
...
subtest 'Scenario 17: all_domains() triggers both pipelines regardless of call order' => sub {
	...
	install_stubs(
		rdns	=> 'mail.sender.example',
		resolve => {
			'urlhost.example'   => '198.51.100.1',
			'mailhost.example'  => '198.51.100.2',
		},
